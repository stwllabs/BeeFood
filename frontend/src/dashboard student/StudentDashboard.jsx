...
const StudentDashboard = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setUser(prev => ({ ...prev, balance: prev.balance - totalBelanja }));
      ...
      triggerToast("🎉 Pre-order berhasil ditempatkan!");
    } catch (err) {
  ...
  const renderCartContent = () => (
    <>
      ...
      <div className="pt-4 mt-auto">
        ...
        >
          <Wallet className="w-4 h-4" /> {actionLoading ? "Memproses..." : "Bayar Sekarang"}
        </button>
      </div>
    </>
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="bg-orange-50 px-4 py-2 rounded-2xl border border-orange-100 flex items-center gap-3">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-black text-orange-600 leading-none">Rp {user?.balance.toLocaleString("id-ID")}</p>
            </div>
          </div>
                ...
                >
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-6">
                      <h2 className="text-xl font-black text-gray-900">Pilihan Restoran Kantin</h2>
                      {searchQuery && (
                        <span className="text-xs font-bold text-orange-500 bg-orange-50 border border-orange-100 px-3 py-1 rounded-full">
                          Ditemukan {filteredTenants.length} Tenant yang Cocok
                        </span>
                      )}
                    </div>
                        ...
                        {filteredTenants.map((tenant) => (
                          ...
                          >
                            ...
                            <div className="p-5 flex-1 flex flex-col justify-between">
                              ...
                              {/* Highlight matching menu search inside card */}
                              {searchQuery && (
                                <div className="mb-3 bg-orange-50/50 p-2 rounded-xl border border-orange-100/50 text-[10px] text-orange-700 font-medium">
                                  Menu cocok: {tenant.menus.filter(m => m.name.toLowerCase().includes(searchQuery.toLowerCase())).map(m => m.name).join(", ")}
                                </div>
                              )}
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg text-white uppercase ${
